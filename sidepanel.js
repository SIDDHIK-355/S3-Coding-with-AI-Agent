...
async function sendChat() {
  ...
  chatEl.innerHTML += `<div class="chat-bubble user-bubble">${escHtml(message)}</div>`;
  const typingId = 'typing_' + Date.now();
  chatEl.innerHTML += `<div class="chat-bubble ai-bubble" id="${typingId}">⏳ thinking...</div>`;
  ...
  try {
    ...
    document.getElementById(typingId).outerHTML =
      `<div class="chat-bubble ai-bubble">${renderMarkdown(reply)}</div>`;
    chatEl.scrollTop = chatEl.scrollHeight;
  } catch (err) {
    document.getElementById(typingId).outerHTML =
      `<div class="chat-bubble ai-bubble error-bubble">❌ ${escHtml(err.message)}</div>`;
  }
